...
v\item [{[A\textsubscript{8}]}] Public transportations have services that allow a user to buy tickets online.
\end{itemize}
...
\item [{[DE\textsubscript{2}]}] The application bases on some other services like Google maps, weather services and traffic services and uses the received information to guarantee its services.
\end{itemize}
...
\end{enumerate} \\ \hline
Output condition& Travlendar+ updates the calendar with the  modified activity, then redirects U on the main screen. If the location has been modified, on the map the activity appears in the new location.
\\ \hline
Exception& The possible exceptions are:
\begin{enumerate}
...
\begin{figure}
\centering
\makebox[\textwidth]{\includegraphics[scale=0.5]{"UseCase Modify activity"}  }
...
\begin{figure}[h]
\makebox{\includegraphics[scale=0.6]{"Alloy/4 - Facts"} }
\end{figure}
\begin{figure}[h]
...
\newpage
\begin{figure}[h]
\makebox{\includegraphics[scale=0.4]{"Alloy/11 - Instance2"} }
\end{figure}
\clearpage
\end{landscape}
...
\newpage
\begin{figure}[h]
\makebox{\includegraphics[scale=0.4]{"Alloy/12 - Instance3"} }
\end{figure}
\clearpage
\end{landscape}
