...
\item [{[G\ped{1}]}]	A user should be able to go to a meeting.
\item [{[G\ped{2}]}]	A user should be able to know if he/she cannot arrive on time to a meeting.
\item [{[G\ped{3}]}]	A user should be able to have a break that has a certain duration between two meetings.
\item [{[G\ped{4}]}]	The time or the location of a meeting can be modified.

\item [{[G\ped{5}]}]  A user should be able to unsay  a meeting.
\item [{[G\ped{6}]}]	A user does not want to use some travel means.
\item [{[G\ped{7}]}]	A user does not want to walk more than a specific distance.
\item [{[G\ped{8}]}] A user should always be able to have a break between two specific times and with a minimum duration.
\item [{[G\ped{9}]}] A user should be able to minimize carbon footprint when he/she goes to a meeting.
\item [{[G\ped{10}]}] A user should be able to know every possible option to go to a meeting with different travel means in order to choose the suitable one.
%\item [{[G\ped{7}]}]	 provide constraints on different travel means;
\item [{[G\ped{11}]}]	A user should be able to buy public transportation tickets or day/week/season pass basing on his/her needs.
\item [{[G\ped{12}]}]	A user would like to see on Tripadvisor advised places for his/her breaks.
\item [{[G\ped{13}]}]	A user would like to locate the nearest vehicle of a vehicle sharing system if he/she wants to use that type of travel means.
\item [{[G\ped{14}]}]	A user would like to have real-time news on strikes, weather and traffic.
...
\item[{[A\ped{1}]}]  Accurate registered user's location is known by GPS.
\item [{[A\ped{2}]}] The user knows the right details about the activities.
\item [{[A\ped{3}]}] The public transportations are always in time.
\item [{[A\ped{4}]}] The external third part services (e.g. vehicle sharing systems, google maps, travel means' websites ecc...) work correctly.
\item [{[A\ped{5}]}] The news about the traffic, the weather and strikes are correct.
\end{itemize}

\paragraph{Dependencies}
\begin{itemize}
\item[{[DE\ped{1}]}]  The application depends on external application and websites for some services, such as: the purchase of public transportation's tickets, the use of vehicle sharing systems. 
\item [{[DE\ped{2}]}] The application bases on some other services like google maps, meteo services and traffic services and uses the received information to guarantee its services.
\end{itemize}

\paragraph{Constraints}
\begin{itemize}
\item [{[C\ped{1}]}] Travlendar+ requires a smartphone or a tablet with internet connection in order to offer its functionalities.
\item [{[C\ped{2}]}] Travlendar+ needs a GPS service to find the correct location of the user.
...
\begin{itemize}
%\item[\textbf{ {[G\ped{1}]}}]	\textbf{	Users should be able to sign up into the application.}
%\begin{itemize}
...
%\item[{[R\ped{2}]}] The system should be able to extract data from Facebook or Twitter if the visitor signs in with that application.
...
%\begin{itemize}
%\item[{[R\ped{1}]}] The system should allow only registered users to log in.
%\item[{[R\ped{2}]}] Username and password inserted must be correct to perform the login.
%\item[{[R\ped{3}]}] Visitors cannot access the calendar and to the map without login.
%\end{itemize}
\item[\textbf{ {[G\ped{1}]}}]	\textbf{	A user should be able to go to a meeting.}
\begin{itemize}
%\item[{[R\ped{1}]}] The system should allow only registered users to create activities.
\item[{[R\ped{1}]}] The system should allow registered users to create activities.
%\item[{[R\ped{2}]}] Users must be logged in the application.
\item[{[R\ped{2}]}] Users must complete the form by filling the mandatory fields and confirm the creation.
\item[{[R\ped{3}]}] Users must specify the type of activity (meeting or break).
\item[{[R\ped{4}]}] The system should provide different form based on the type of the activity.
...
\end{itemize}
\item[\textbf{ {[G\ped{4}]}}]	\textbf{	The time or the location of a meeting can be modified.}
\begin{itemize}
...
\item[{[R\ped{4}]}] Modified data will not be available any more after the update.
\end{itemize}
...
\item[{[R\ped{7}]}]  The system should be able to provide a solution to have a break if a user specifies it.
...
\begin{itemize}
\item[{[R\ped{1}]}]  Users should be registered and logged in.
\item[{[R\ped{2}]}]  The system should be able to open the application or the website of the service that provides that type of trip.
...
\end{itemize}
\item[\textbf{ {[G\ped{9}]}}]	\textbf{	Users should be able to see on Tripadvisor advised places when they insert new breaks.}
\begin{itemize}
\item[{[R\ped{1}]}]  Users should be registered and logged in.
\item[{[R\ped{2}]}]  The system should be able to open the application or the website of Tripadvisor.
\item[{[R\ped{3}]}] Users must confirm if they want to open an external application.
\end{itemize}
...
\item[{[R\ped{4}]}] The system should be able to calculate the necessary time for each trip option.
\end{itemize}
...
\begin{itemize}
\item[{[R\ped{1}]}]  Users should be registered and logged in.
\item[{[R\ped{2}]}]  The system should be able to open the application or the website of the service.
\item[{[R\ped{3}]}] Users must confirm if they want to open an external application.
\item[{[D\ped{1}]}]  The chosen service must have an application or a website.
\end{itemize}
...
\end{itemize}
\item[\textbf{ {[G\ped{11}]}}]	\textbf{	Users should be able to know problems that can cause changes in trip.}
\begin{itemize}
\item[{[R\ped{1}]}]  Users should be registered and logged in.
...
\item[{[D\ped{1}]}]  The real-time information must be correct and provided by reliable sites.
\end{itemize}
\end{itemize}
